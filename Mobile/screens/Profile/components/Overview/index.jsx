...
export default function Overview({ styles, navigation }) {
  ...
  return (
    <View style={{ flex: 1, height: screenHeight * 0.45 }}>
      ...
      <Image
        resizeMode="contain"
        style={{ width: screenWidth, height: screenHeight * 0.322 }}
        source={wallpaper}
      />
      <View
        style={[
          styles.avatarWrapper,
          { top: screenHeight * 0.24, left: screenWidth * 0.3 },
        ]}
      >
        <Image
          style={styles.avatarImg}
          resizeMode="contain"
          source={user?.avatar ? user?.avatar : avatar}
        />
        <Text style={styles.avatarName}>{user ? user.username : ''}</Text>
      </View>
    </View>
  )
}
